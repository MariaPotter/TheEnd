...
int main ()
{
    ...
    for(i = 1; i < N; i++)
    {
        for(j = 1; j < N; j++)
        {
            for(k = 1; k < N; k++)
            {
                F[(i-1)*m+(j-1)*(N-1)+k-1] = f(M_PI*i/N, M_PI*j/N, M_PI*k/N);
                U[(i-1)*m+(j-1)*(N-1)+k-1] = u(M_PI*i/N, M_PI*j/N, M_PI*k/N);
            }
        }
    }
    
    p = fftw_plan_r2r_3d(n, n, n, F, G, FFTW_RODFT00, FFTW_RODFT00, FFTW_RODFT00, FFTW_ESTIMATE);
...
void change_F(double* G, double* F)
{
    ...
    for(i = 0; i < N - 1; i++)
    {
        ...
        {
            for(k = 0; k < N - 1; k++)
                F[i*m+j*(N-1)+k] = G[i*m+j*(N-1)+k] / (pow(M_PI,-2)*pow(2*N,5)*(c[i]+c[j]+c[k]));
        }
    }
}
